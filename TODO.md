...
- [x] Embeddings + retrieval foundations:
  - [x] Add embeddings storage and retrieval (pgvector in Postgres or external vector DB).
  - [x] Extend KB ingestion to generate and store chunk embeddings with metadata (title, section path, source URL).
  - [x] Implement retrieval in the enricher: embed ticket, fetch top-k chunks, inject into Claude prompt, strict JSON/schema validation.
- [x] Enrichment quality + grounding:
  - [x] Prompt upgrade: strict response template (acknowledge → 2–4 bullet steps → next step).
  - [x] Enforce “use KB only; if missing, ask 1–2 clarifying questions.”
  - [x] KB context discipline: keep top-k small (5–8) and cap context length.
  - [x] Output normalization + schema: enums for category/sentiment and post-output normalization.
- [x] Citations + provenance:
  - [x] Store chunk IDs/titles from retrieval.
  - [x] Include citations: [{chunk_id, title, heading_path}].
  - [x] Add citations array to enriched event schema.
  - [x] Persist citations JSON in enriched_tickets.
...
- [x] Ensure at-least-once semantics by committing offsets only after DB write.
